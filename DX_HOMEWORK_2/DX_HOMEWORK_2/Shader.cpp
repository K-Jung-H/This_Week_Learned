...
void CShader::OnPrepareRender(ID3D12GraphicsCommandList* pd3dCommandList, bool bWireframe)
{
...
void CShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, bool Draw_Wire)
{
	OnPrepareRender(pd3dCommandList, Draw_Wire);
}
...
void CObjectsShader::ReleaseObjects() // 사용 안함 Scene에서 모두 처리
{
	if (m_ppObjects)
	{
...
void CObjectsShader::AnimateObjects(float fTimeElapsed) // 사용 안함 Scene에서 모두 처리
{
	for (int j = 0; j < m_nObjects; j++)
	{
...
void CObjectsShader::ReleaseUploadBuffers() // 사용 안함 Scene에서 모두 처리
{
	if (m_ppObjects)
	{
		for (int j = 0; j < m_nObjects; j++) 
...
void CObjectsShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, bool Draw_Wire) 
{
	CShader::Render(pd3dCommandList, pCamera, Draw_Wire);
