...
void CGameFramework::BuildObjects(int scene_num)
{
	...
	{
		if (m_pScene) {
			m_pScene->BuildObjects(m_pd3dDevice, m_pd3dCommandList, 1);
			m_pScene->scene_num = 1;
		}
		scene_mode = 1;
	}
	else if (scene_num == 2) // 인 게임
	{
		if (m_pScene) {
			m_pScene->BuildObjects(m_pd3dDevice, m_pd3dCommandList, 2);
...
void CGameFramework::Change_Scene_Objects()
{
	ReleaseObjects();
	if (scene_mode ==1)
	{
...
void CGameFramework::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_RBUTTONDOWN:
		...
		if (m_pSelectedObject != NULL)
		{
			...
			m_pSelectedObject->picked = true;
			if (scene_mode == 1)
			{
				m_pSelectedObject->Draw_Wire = true;
			}
...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case 'Z':
			...
			((CAirplanePlayer*)m_pPlayer)->barrier_mode =  true;
			((CAirplanePlayer*)m_pPlayer)->m_pBarrier->barrier_mode = true;
			((CAirplanePlayer*)m_pPlayer)->m_pBarrier->life = 2;
			break;

		case VK_TAB:
			Change_Scene_Objects();
			break;
		...
		case '1':
			((CAirplanePlayer*)m_pPlayer)->m_pBarrier->life = 1;
			((CAirplanePlayer*)m_pPlayer)->m_pBarrier->m_bBlowingUp = true;
...
void CGameFramework::AnimateObjects()
{
	...
	if (m_pCamera != NULL)
	{
		m_pScene->CheckPlayerByWallCollision(m_pCamera->GetMode());
	}
	

	m_pScene->CheckObjectByObjectCollisions();
	m_pScene->CheckObjectByWallCollisions();
	m_pScene->CheckObjectByBulletCollisions();

	if (scene_mode == 2)
	{
		if (static_cast<CAirplanePlayer*>(m_pPlayer)->barrier_mode)
			m_pScene->CheckBarrierByBulletCollisions();
		else
			m_pScene->CheckPlayerByBulletCollisions();
	}
...
void CGameFramework::FrameAdvance()
{
	...
	if (m_pScene) 
		m_pScene->Render(m_pd3dDevice, m_pd3dCommandList, m_pCamera);
