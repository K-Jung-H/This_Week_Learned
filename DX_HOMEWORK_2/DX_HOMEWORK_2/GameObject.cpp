...
void CGameObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	if (IsVisible(pCamera))
	{
		...
		if (m_pShader) 
			m_pShader->Render(pd3dCommandList, pCamera, Draw_Wire);
		if (m_pMesh) 
...
void CExplosiveObject::PrepareExplosion(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	m_pExplosionMesh = new CCubeMeshDiffused(pd3dDevice, pd3dCommandList, 0.5f, 0.5f, 0.5f,
		{ 0.5f, 0.0f, 0.0f, 1.0f }, { 0.7f, 0.0f, 0.0f, 1.0f });
}
...
CEnemyBulletObject::CEnemyBulletObject(float fEffectiveRange) : CBulletObject(fEffectiveRange)
{
	m_fDuration = 1.0f;
}
CEnemyBulletObject::~CEnemyBulletObject()
{

}

void CEnemyBulletObject::Animate(float fElapsedTime)
{
	if (crashed)
	{
		m_fElapsedTimes += fElapsedTime;

		if (m_fElapsedTimes <= 0.5f)
		{
			m_bBlowingUp = true;
			XMFLOAT3 xmf3Position = GetPosition();
			for (int i = 0; i < 10; i++)
	...
	CBulletObject::Animate(fElapsedTime);
}


/////////////////////////////////////////////////////////////////////////////////////////////////////

CEnemyObject::CEnemyObject(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	CSphereMeshDiffused* pBulletMesh = new CSphereMeshDiffused(pd3dDevice, pd3dCommandList, 2.0f, 20, 20, XMFLOAT4(1.0f, 0.0f, 1.0f, 0.0f));
	for (int i = 0; i < E_BULLETS; i++)
	{
		m_ppBullets[i] = new CEnemyBulletObject(m_fBulletEffectiveRange);
		m_ppBullets[i]->SetMesh(pBulletMesh);
		m_ppBullets[i]->SetRotationAxis(XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_ppBullets[i]->SetRotationSpeed(360.0f);
		m_ppBullets[i]->SetMovingSpeed(30.0f);
		m_ppBullets[i]->SetActive(false);
		m_ppBullets[i]->crashed = false;
	}
	m_fDuration = 0.5f;
	m_fExplosionSpeed = 50.0f;

	Fire_Period = rand() % 3 + 1;
}
...
void CEnemyObject::Animate(float fElapsedTime)
{
	...
	cool_down += fElapsedTime;
	if (Fire_Period <= cool_down)
	{
		FireBullet();
		cool_down = 0;
	}
...
void CEnemyObject::FireBullet()
{

	CEnemyBulletObject* eBulletObject = NULL;
	for (int i = 0; i < E_BULLETS; i++)
	{
		if (!m_ppBullets[i]->m_bActive && !m_ppBullets[i]->crashed)
		{
			eBulletObject = m_ppBullets[i];
			break;
		}
	}
	...
	if (eBulletObject)
	{
		...
		eBulletObject->m_xmf4x4World = m_xmf4x4World;

		eBulletObject->SetFirePosition(xmf3FirePosition);
		eBulletObject->SetMovingDirection(xmf3Direction);
		eBulletObject->SetActive(true);
		eBulletObject->crashed = false;
...
CBarrierObject::CBarrierObject(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	CBarrierMeshDiffused* pBarrierMesh = new CBarrierMeshDiffused(pd3dDevice, pd3dCommandList, 20.0f, 20, 20, XMFLOAT4(0.8f, 0.5f, 0.0f, 0.0f));
	SetMesh(pBarrierMesh);
	m_pExplosionMesh = new CCubeMeshDiffused(pd3dDevice, pd3dCommandList, 0.5f, 0.5f, 0.5f,
		{ 0.8f, 0.5f, 0.0f, 1.0f }, { 0.8f, 0.5f, 0.0f, 0.0f });

	m_xmBSphere.Radius = 20.0f;
	m_fExplosionSpeed = 15.0f;
...
void CBarrierObject::Animate(float fElapsedTime, XMFLOAT3 p_pos)
{
