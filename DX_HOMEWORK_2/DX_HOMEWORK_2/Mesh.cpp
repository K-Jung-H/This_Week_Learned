...
CSphereMeshDiffused::CSphereMeshDiffused(ID3D12Device* pd3dDevice,
	ID3D12GraphicsCommandList* pd3dCommandList, float fRadius, int nSlices, int nStacks, XMFLOAT4 xmf4Color) :
	CMesh(pd3dDevice, pd3dCommandList)
{
	...
	//구의 위(북극)를 나타내는 정점이다. 
	m_pVertices[k++] = CDiffusedVertex(0.0f, +fRadius, 0.0f, Vector4::Add(xmf4Color, RANDOM_COLOR));
	...
	for (int j = 1; j < nStacks; j++)
	{
		...
		for (int i = 0; i < nSlices; i++)
		{
			...
			m_pVertices[k++] = CDiffusedVertex(fRadius * sinf(phi_j) * cosf(theta_i),
				fRadius * cosf(phi_j), fRadius * sinf(phi_j) * sinf(theta_i), Vector4::Add(xmf4Color, RANDOM_COLOR));
		}
	}
...
CBarrierMeshDiffused::CBarrierMeshDiffused(ID3D12Device* pd3dDevice,
	ID3D12GraphicsCommandList* pd3dCommandList, float fRadius, int nSlices, int nStacks, XMFLOAT4 xmf4Color) :
	CMesh(pd3dDevice, pd3dCommandList)
{
	m_nStride = sizeof(CDiffusedVertex);
...
CBarrierMeshDiffused::~CBarrierMeshDiffused()
{
}
