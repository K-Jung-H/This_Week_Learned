...
void CGameFramework::CreateDirect2DDevice()
{
	...
	hResult = m_pdWriteFactory->CreateTextFormat(L"맑은 고딕", NULL, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, 20.0f, L"ko-KR", &m_pdw_Score_Font);
...
void CGameFramework::BuildObjects()
{
	...
	if (rendering_scene) rendering_scene->ReleaseUploadBuffers();
...
void CGameFramework::ProcessInput()
{
	...
	if (GetKeyboardState(pKeysBuffer) && rendering_scene) bProcessedByScene = rendering_scene->ProcessInput(pKeysBuffer);
