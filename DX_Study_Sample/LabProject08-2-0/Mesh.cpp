...
CHeightMapImage::CHeightMapImage(LPCTSTR pFileName, int nWidth, int nLength, XMFLOAT3 xmf3Scale)
{
	...
	WORD*pHeightMapPixels = new WORD[m_nWidth * m_nLength];

	HANDLE hFile = ::CreateFile(pFileName, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_ATTRIBUTE_READONLY, NULL);
	DWORD dwBytesRead;
	::ReadFile(hFile, pHeightMapPixels, (m_nWidth * m_nLength)* sizeof(WORD), &dwBytesRead, NULL);
	::CloseHandle(hFile);

	m_pHeightMapPixels = new WORD[m_nWidth * m_nLength];
...
{
	CHeightMapImage *pHeightMapImage = (CHeightMapImage *)pContext;
	WORD*pHeightMapPixels = pHeightMapImage->GetHeightMapPixels();
