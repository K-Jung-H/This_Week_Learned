...
D3D12_GPU_DESCRIPTOR_HANDLE CScene::CreateConstantBufferView(ID3D12Device* pd3dDevice, ID3D12Resource* pd3dConstantBuffer, UINT nStride)
{
	// 상수 버퍼 뷰 설명 구조체 초기화
	D3D12_CONSTANT_BUFFER_VIEW_DESC d3dCBVDesc;
	d3dCBVDesc.SizeInBytes = nStride; // 상수 버퍼의 크기 설정
	d3dCBVDesc.BufferLocation = pd3dConstantBuffer->GetGPUVirtualAddress(); // GPU 가상 주소 설정

	// 상수 버퍼 뷰 생성: d3dCBVDesc에 정의된 대로 m_pDescriptorHeap->m_d3dCbvCPUDescriptorNextHandle 위치에 CBV 생성
	pd3dDevice->CreateConstantBufferView(&d3dCBVDesc, m_pDescriptorHeap->m_d3dCbvCPUDescriptorNextHandle);

	// 현재 GPU 핸들 저장: 이후에 루트 시그니처에 연결할 때 사용
	D3D12_GPU_DESCRIPTOR_HANDLE d3dCbvGPUDescriptorHandle = m_pDescriptorHeap->m_d3dCbvGPUDescriptorNextHandle;

	// 다음 CBV가 저장될 위치를 업데이트
	m_pDescriptorHeap->m_d3dCbvCPUDescriptorNextHandle.ptr += ::gnCbvSrvDescriptorIncrementSize;
	m_pDescriptorHeap->m_d3dCbvGPUDescriptorNextHandle.ptr += ::gnCbvSrvDescriptorIncrementSize;
...
{
	// nDescriptorHeapIndex 번째 힙에 접근
	  // SRV의 CPU 핸들을 nDescriptorHeapIndex에 따라 이동시킴
	m_pDescriptorHeap->m_d3dSrvCPUDescriptorNextHandle.ptr += (::gnCbvSrvDescriptorIncrementSize * nDescriptorHeapIndex);
	// SRV의 GPU 핸들을 nDescriptorHeapIndex에 따라 이동시킴
...
void CScene::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	XMFLOAT3 xmf3Scale(8.0f, 0.1f, 8.0f);
...
#else
	m_pTerrain = new CHeightMapTerrain(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, _T("Image/Heightmap_2K_A.raw"), 2048, 2048, 2048, 2048, xmf3Scale, xmf4Color);
#endif
	m_nShaders = 1;
...
ID3D12RootSignature *CScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	d3dSamplerDesc.AddressU = D3D12_TEXTURE_ADDRESS_MODE_WRAP; // D3D12_TEXTURE_ADDRESS_MODE_WRAP
