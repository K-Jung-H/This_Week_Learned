...
VS_TERRAIN_OUTPUT VSTerrain(VS_TERRAIN_INPUT input)
{
    ...
    output.lava = 0; // 기본값 설정 (false)

    if (input.position.y < 900)
        output.lava = 1.0f; // lava를 true로 설정
...
//	float fAlpha = gtxtTerrainAlphaTexture.Sample(gSamplerState, input.uv0);
    ...
    if (input.lava == 1.0f)
        cColor = cDetailTexColors[2];
	else
        cColor += float4(0.1f, 0.1f, 0.1f, 0.0f);
    // lerp(cDetailTexColors[1] * 0.25f, cDetailTexColors[2], 1.0f - fAlpha);
