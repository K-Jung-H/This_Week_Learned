...
ID3D12RootSignature* CScene::UI_CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
	ID3D12RootSignature* pd3dGraphicsRootSignature = NULL;
...
void CScene::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	m_pd3d_ui_GraphicsRootSignature = UI_CreateGraphicsRootSignature(pd3dDevice);
	m_n_ui_Shaders = 1;

	m_ui_Shaders = new UIShader[m_n_ui_Shaders];
	m_ui_Shaders[0].CreateShader(pd3dDevice, m_pd3dGraphicsRootSignature);
	m_ui_Shaders[0].BuildObjects(pd3dDevice, pd3dCommandList);
...
void CScene::UI_Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	pd3dCommandList->SetGraphicsRootSignature(m_pd3d_ui_GraphicsRootSignature); 

	for (int i = 0; i < m_n_ui_Shaders; i++)
	{
		m_ui_Shaders[i].Render(pd3dCommandList, NULL);
	}
}

void CScene::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	pd3dCommandList->SetGraphicsRootSignature(m_pd3dGraphicsRootSignature); // 그래픽 루트 시그너쳐를 파이프라인에 연결
	pCamera->SetViewportsAndScissorRects(pd3dCommandList); // 뷰포트 / 씨저 사각형 설정
	...
	UI_Render(pd3dCommandList, pCamera);
