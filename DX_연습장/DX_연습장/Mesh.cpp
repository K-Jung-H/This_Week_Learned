...
void CMesh::ReleaseUploadBuffers()
{
	if (m_pd3dVertexUploadBuffer) 
		m_pd3dVertexUploadBuffer->Release(); // 정점 버퍼를 위한 업로드 버퍼 소멸 
	m_pd3dVertexUploadBuffer = NULL;

	if (m_pd3dIndexUploadBuffer) 
		m_pd3dIndexUploadBuffer->Release();
	m_pd3dIndexUploadBuffer = NULL; // 인덱스 버퍼 소멸 
};
...
void CMesh::Render(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	if (m_pd3dIndexBuffer) // 메쉬가 인덱스 버퍼를 갖고 있다면
	{
		pd3dCommandList->IASetIndexBuffer(&m_d3dIndexBufferView); // 인덱스 버퍼 연결
		pd3dCommandList->DrawIndexedInstanced(m_nIndices, 1, 0, 0, 0); // 인덱스를 사용하여 그리기
	}
	else
	{
		// 메쉬의 정점 버퍼 뷰를 렌더링 == 파이프라인(입력 조립기)을 작동
	...
	float fWidth, float fHeight, float fDepth) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	// 삼각형 리스트 활용 --> 각 면은 2개의 삼각형을 가지고 각 삼각형은 3개의 정점이 필요 
	// ==> 인덱스 버퍼는 전체 36개의 인덱스
	m_nIndices = 36;
	UINT pnIndices[36];

	pnIndices[0] = 3; pnIndices[1] = 1; pnIndices[2] = 0; // 앞면 사각형의 위쪽 삼각형
	pnIndices[3] = 2; pnIndices[4] = 1; pnIndices[5] = 3; // 앞면 사각형의 아래쪽 삼각형

	pnIndices[6] = 0; pnIndices[7] = 5; pnIndices[8] = 4; // 윗면 사각형의 위쪽 삼각형
	pnIndices[9] = 1; pnIndices[10] = 5; pnIndices[11] = 0; // 윗면 사각형의 아래쪽 삼각형

	pnIndices[12] = 3; pnIndices[13] = 4; pnIndices[14] = 7; // 뒷면 사각형의 위쪽 삼각형
	pnIndices[15] = 0; pnIndices[16] = 4; pnIndices[17] = 3; // 뒷면 사각형의 아래쪽 삼각형

	pnIndices[18] = 1; pnIndices[19] = 6; pnIndices[20] = 5; // 아래면 사각형의 위쪽 삼각형
	pnIndices[21] = 2; pnIndices[22] = 6; pnIndices[23] = 1; // 아래면 사각형의 아래쪽 삼각형

	pnIndices[24] = 2; pnIndices[25] = 7; pnIndices[26] = 6; // 옆면 사각형의 위쪽 삼각형
	pnIndices[27] = 3; pnIndices[28] = 7; pnIndices[29] = 2; // 옆면 사각형의 아래쪽 삼각형

	pnIndices[30] = 6; pnIndices[31] = 4; pnIndices[32] = 5; // 옆면 사각형의 위쪽 삼각형
	pnIndices[33] = 7; pnIndices[34] = 4; pnIndices[35] = 6; // 옆면 사각형의 아래쪽 삼각형
