...
void CGameFramework::BuildObjects()
{
	...
	m_UICamera = new CFirstPersonCamera(m_UICamera);
	m_UICamera->SetTimeLag(0.0f);
	m_UICamera->SetOffset(XMFLOAT3(0.0f, 20.0f, 0.0f));
	
	m_UICamera->SetViewport(FRAME_BUFFER_WIDTH * 0.75f, 0, FRAME_BUFFER_WIDTH * 0.25f, FRAME_BUFFER_HEIGHT * 0.15f, 0.0f, 1.0f);
	m_UICamera->SetScissorRect(FRAME_BUFFER_WIDTH * 0.75f, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT * 0.15f);
	m_UICamera->GenerateProjectionMatrix(1.0f, 500.0f, ASPECT_RATIO, 90.0f);
	m_UICamera->GenerateViewMatrix(XMFLOAT3(0.0f, 0.0f, -2.0f), XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(0.0f, 1.0f, 0.0f));
...
void CGameFramework::FrameAdvance()
{
	...
	if (m_pScene)
	{
		...
		// 깊이 버퍼를 1.0으로 지우고 렌더링 하게 해서 다음 그리는 내용은 무조건 그려지게 하기
		m_pd3dCommandList->ClearDepthStencilView(d3dDsvCPUDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);

		if (m_UICamera)
			m_pScene->UI_Render(m_pd3dCommandList, m_UICamera);
