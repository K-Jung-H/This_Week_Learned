...
{
	//카메라의 위치 벡터, 카메라가 바라보는 지점, 카메라의 Up 벡터
	m_xmf4x4View = Matrix4x4::LookAtLH(m_xmf3Position, m_xmf3LookAtWorld, m_xmf3Up);
}

void CCamera::RegenerateViewMatrix()
{
	//카메라의 z축을 기준으로 카메라의 좌표축들이 직교하도록 카메라 변환 행렬을 갱신

	// 카메라의 z축 벡터를 정규화 
	m_xmf3Look = Vector3::Normalize(m_xmf3Look); 	

	// 카메라의 z축과 y축에 수직인 벡터를 x-축으로 설정
	m_xmf3Right = Vector3::CrossProduct(m_xmf3Up, m_xmf3Look, true); 
	
	//카메라의 z축과 x축에 수직인 벡터를 y-축으로 설정
...
void CCamera::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	m_pd3dcbCamera = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes, 
...
void CCamera::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	//VS_CB_CAMERA_INFO* pcbMappedCamera = (VS_CB_CAMERA_INFO*)(m_pcbMappedCamera);
...
void CSpaceShipCamera::Rotate(float x, float y, float z)
{
	if (m_pPlayer && (x != 0.0f))
	{
		...
		// 카메라의 위치 벡터에서 플레이어의 위치 벡터 빼기 
		// 결과는 플레이어 위치를 기준(원점)으로 한 카메라의 위치 벡터
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, m_pPlayer->GetPosition());

		// 플레이어의 위치를 중심으로 카메라의 위치 벡터(플레이어를 기준으로 한)를 회전 
		m_xmf3Position = Vector3::TransformCoord(m_xmf3Position, xmmtxRotate);

		// 회전시킨 카메라의 위치 벡터에 플레이어의 위치를 더하여 카메라의 위치 벡터 구하기
...
CFirstPersonCamera::CFirstPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		// 1인칭 카메라로 변경하기 이전의 카메라가 스페이스-쉽 카메라이면 카메라의 Up 벡터를 월드좌표의 y축이 되도록 함
		// 이것은 스페이스-쉽 카메라의 로컬 y-축 벡터가 어떤 방향이든지 1인칭 카메라(대부분 사람인 경우)의 로컬 y축 == 월드좌표 y축 되도록
		// 즉, 똑바로 서있는 형태로 설정 
		
		// 로컬 x-축 벡터와 로컬 z축 벡터의 y-좌표가 0.0f가 되도록 함
		// 이것은 다음 그림과 같이 로컬 x축 벡터와 로컬 z축 벡터를 xz평면(지면)으로 투영하는 것을 의미
		// 즉, 1인칭 카메라의 로컬 x축 벡터와 로컬 z축 벡터는 xz평면에 평행
...
CThirdPersonCamera::CThirdPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		// 3인칭 카메라로 변경하기 이전의 카메라가 스페이스-쉽 카메라이면 카메라의 Up 벡터를 월드좌표의 y축이 되도록 해야 함
		// 스페이스-쉽 카메라의 로컬 y축 벡터가 어떤 방향이든지 
		// 3인칭 카메라(대부분 사람인 경우)의 로컬 y축 벡터가 월드좌표의 y축이 되도록 똑바로 서있는 형태로 설정 
		
		// 로컬 x축 벡터와 로컬 z축 벡터의 y좌표가 0.0f가 되도록 함 
		// --> 로컬 x축 벡터와 로컬 z축 벡터를 xz평면(지면)으로 투영하는 것
		// --> 3인칭 카메라의 로컬 x축 벡터와 로컬 z축 벡터는 xz평면에 평행
		if (pCamera->GetMode() == SPACESHIP_CAMERA)
		{
			m_xmf3Up = XMFLOAT3(0.0f, 1.0f, 0.0f);
			m_xmf3Right.y = 0.0f;
...
void CThirdPersonCamera::Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	...
	if (m_pPlayer)
	{
		...
		// m_fTimeLag가 1보다 크면 fTimeLagScale이 작아지고 실제 회전(이동)이 적게 일어날 것 
		
		// m_fTimeLag가 0이 아닌 경우 fTimeElapsed를 곱하고 있으므로 
		// 3인칭 카메라는 1초의 시간동안(1.0f / m_fTimeLag)의 비율만큼 플레이어의 회전을 따라가게 될 것
