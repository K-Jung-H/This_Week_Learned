...
protected:
	...
	XMFLOAT3 m_xmf3Position; 	// 카메라의 위치(월드좌표계) 벡터
	...
	float m_fPitch; // 카메라가 x축으로 회전한 각도
	float m_fRoll; // 카메라가 y축으로 회전한 각도
	float m_fYaw; // 카메라가 z축으로 회전한 각도

	DWORD m_nMode; // 카메라의 종류(1인칭 카메라, 스페이스-쉽 카메라, 3인칭 카메라)를 나타냄
	XMFLOAT3 m_xmf3LookAtWorld; // 플레이어가 바라볼 위치 벡터, 주로 3인칭 카메라에서 사용
	XMFLOAT3 m_xmf3Offset; // 플레이어와 카메라의 오프셋을 나타내는 벡터, 주로 3인칭 카메라에서 사용
	
	float m_fTimeLag; // 플레이어가 회전할 때 카메라 회전 지연 시간
	XMFLOAT4X4 m_xmf4x4View;
	XMFLOAT4X4 m_xmf4x4Projection;
	...
	CPlayer *m_pPlayer = NULL; 	// 카메라를 가지고 있는 플레이어 포인터
...
public:
	...
	void GenerateViewMatrix();// 카메라 변환 행렬을 생성
	void GenerateViewMatrix(XMFLOAT3 xmf3Position, XMFLOAT3 xmf3LookAt, XMFLOAT3 xmf3Up);

	// 카메라가 여러번 회전을 하게 되면 누적된 실수 연산의 부정확성 때문
	// 카메라의 로컬 x축(Right), y축(Up), z축(Look)이 서로 직교하도록 만듬
	...
	//3인칭 카메라에서 카메라가 바라보는 지점을 설정 -> 플레이어를 바라보도록 설정
