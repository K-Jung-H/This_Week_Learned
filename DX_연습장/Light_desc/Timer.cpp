...
void CGameTimer::Tick(float fLockFPS)
{
	...
	float fTimeElapsed = (m_nCurrentTime - m_nLastTime) * m_fTimeScale; // 마지막으로 이 함수를 호출한 이후 경과한 시간을 계산 
	if (fLockFPS > 0.0f) // fLockFPS가 0보다 크면 이 시간만큼 호출한 함수를 기다림 
	{
		while (fTimeElapsed < (1.0f / fLockFPS)) 
		{
			if (m_bHardwareHasPerformanceCounter)
				...
				m_nCurrentTime = ::GetTickCount64();
			fTimeElapsed = (m_nCurrentTime - m_nLastTime) * m_fTimeScale; //마지막으로 이 함수를 호출한 이후 경과한 시간을 계산
		}
	}
	m_nLastTime = m_nCurrentTime; // 현재 시간을 m_nLastTime에 저장
